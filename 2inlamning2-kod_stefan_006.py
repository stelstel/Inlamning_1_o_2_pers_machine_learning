...
accuracy = accuracy_score(y_test, y_pred)
results = store_results(results, "MultiOutputClassifier", f"{round(accuracy * 100, 3)} %")
...
grid = GridSearchCV(multi_logreg, param_grid, cv=7, scoring='accuracy')
grid.fit(x_train, y_train)
results = store_results(results, "MultiOutputClassifier", f"{round(float(grid.best_score_) * 100, 3)} %", "GridSearchCV", grid.best_params_)
