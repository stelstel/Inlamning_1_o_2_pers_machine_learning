...
accuracy = accuracy_score(y_test, y_pred)
results = store_results(results, "MultiOutputClassifier", accuracy)
...
grid.fit(x_train, y_train)
results = store_results(results, "MultiOutputClassifier", grid.best_score_, "GridSearchCV", grid.best_params_)
...
random_search = RandomizedSearchCV(multi_logreg, param_distributions=param_dist, n_iter=100, cv=10, scoring='accuracy', random_state=42) # TODO change cv to 5+ and n_iter to higher
random_search.fit(x_train, y_train)

results = store_results(results, "MultiOutputClassifier", random_search.best_score_, "RandomizedSearchCV", random_search.best_params_)
...
# results.sort(reverse=False, key=lambda e:['Accuracy'])
