...
accuracy = accuracy_score(y_test, y_pred)
results = store_results(results, "MultiOutputClassifier", round(accuracy, 3))
...
grid.fit(x_train, y_train)
results = store_results(results, "MultiOutputClassifier", round(float(grid.best_score_), 3), "GridSearchCV", grid.best_params_)
